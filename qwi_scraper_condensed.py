...
function zoom_graph(points) {
	// 	Tasks:
	// 	DONE 1: Calculate new axes based on 'points' only
	// 	DONE 2: change all axis labelling 
	// 	3: zoom points, with some flying off screen (clipping)
	// 	4: set up 'unclicking'
	var elements2=document.querySelectorAll('circle#graph_point')
	zoom_data=[]
	for(var i=0;i<points.length;i++){
		var point=Snap(points[i])
		zoom_data.push([data_points[point.node.attributes.key1.value][point.node.attributes.key2.value][0],data_points[point.node.attributes.key1.value][point.node.attributes.key2.value][1]])
	}
	axes(zoom_data)
	
	for (var i=0; i<elements2.length; i++) {
		var item=Snap(elements2[i])
		new_coords=translate_points([data_points[item.node.attributes.key1.value][item.node.attributes.key2.value][0],data_points[item.node.attributes.key1.value][item.node.attributes.key2.value][1]])
		/*console.log(new_coords)*/
		item.animate({cx:new_coords[0],cy:new_coords[1]},1000)
		graph.append(item)
	}
			
	setTimeout(function() {
		for (var i=0; i<points.length; i++) {
			var item=Snap(points[i])
			item.attr({opacity:1})
			new_coords=translate_points([data_points[item.node.attributes.key1.value][item.node.attributes.key2.value][0],data_points[item.node.attributes.key1.value][item.node.attributes.key2.value][1]])

			x3=Math.round(parseInt(new_coords[0])-27)+.5
			y3=Math.round(parseInt(new_coords[1])-35)+.5
			small_rect=graph.rect(x3,y3,55,21,4,4).attr({opacity:.5,fill:'white',stroke:'black','stroke-width':.8})
			small_text=graph.text(x3+6,y3+14,item.node.attributes.key2.value).attr(
				{'font-weight':400,'font-size':11,'font-family':'Lato'})
			small_group.append(small_rect)
			small_group.append(small_text)
		}
	},1000)
		
		graph.append(small_group)
}